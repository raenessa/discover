# R. Walker, created April 29 2023, last updated July 18 2023
...
def nail(num):
    ...
    if os.path.isfile('/Users/raenessawalker/Desktop/research/eventfiles/'+path)==False:
        subprocess.Popen(["wget",url], cwd=r'/Users/raenessawalker/Desktop/research/eventfiles')
    ########################### variable setup
    path = "KMTC"+fieldno[3:5]+"_I.pysis"
    df = pd.DataFrame(data=pd.read_csv('/Users/raenessawalker/Desktop/research/eventfiles/'+path,sep='\s+', skiprows=1, names=['HJD','Delta_flux','flux_err','mag','mag_err','fwhm','sky','secz']))
    x = df.HJD
    y = df.mag
    yerr = df.mag_err
    sub_hjd = float(df.HJD[0])-50. # subtracted from t0 and jd, makes numbers more manageable 
    ...
    for j in np.arange(0,6,1):
        ...
        #print(df_forpred.HJD[0]-sub_hjd)
        pred_popt,pred_pcov = opt.curve_fit(logm,df_forpred.HJD-sub_hjd,df_forpred.mag,p0=pred_loginit,sigma=df_forpred.mag_err, bounds=([0., 1., -4., 12., -3.], [150., 2.5, 1., 25., 0.])) # order: t0, log(tE), log(u0), m0, log(fs)
        plt.plot(x,logm(x-sub_hjd,*pred_popt), alpha=.7, linewidth=var*.5)
